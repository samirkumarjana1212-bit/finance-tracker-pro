...
async function renderHealth() {
  ...
  const breakdown = document.querySelector('.health-breakdown'); if(breakdown) breakdown.innerHTML = items.map(m=>`<div class='health-factor'><div class='factor-header'><span class='factor-name'>${m.name}</span><span class='factor-score' style='color:${m.score>=80?'var(--success)':m.score>=60?'var(--accent)':m.score>=40?'var(--warning)':'var(--danger)'}'>${Math.round(m.score)}</span></div><div class='progress-bar'><div class='progress-fill ${m.score>=80?'safe':m.score>=60?'':m.score>=40?'warning':'danger'}' style='width:${m.score}%'></div></div><div class='factor-desc'>${m.desc}</div></div>`).join('');
}
